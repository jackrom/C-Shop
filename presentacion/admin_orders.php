  ...
  public function init()
  {
    ...
    if (isset ($_GET['submitByCustomer']))
    {
      if (empty ($_GET['customer_id']))
        $this->mErrorMessage = 'Ning&uacute;n cliente ha sido seleccionado';
      else
      {
    ...
    if (isset ($_GET['submitByOrderId']))
    {
      if (empty ($_GET['orderId']))
        $this->mErrorMessage = 'Debes ingresar la ID de la orden.';
      else
      {
    ...
    if (is_array($this->mOrders) && count($this->mOrders) == 0)
      $this->mErrorMessage =
        'Ninguna orden encontrada que coincida con tu criterio de busqueda!';
