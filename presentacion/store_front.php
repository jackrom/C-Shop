...
class StoreFront
{
  ...
  public function __construct()
  {
    ...
    $this->mPageDescription = "Soluciones que convencen";
  }
  ...
  {
  	$page_description = 'Temas y aplicaciones basadas en HTML5';
  
  	if (isset ($_GET['DepartmentId']) && isset ($_GET['CategoryId']))
  	{
  		$page_description = Catalog::GetDepartmentName($_GET['DepartmentId']) . ' - ' .
  				Catalog::GetCategoryName($_GET['CategoryId']);
  
  		if (isset ($_GET['Page']) && ((int)$_GET['Page']) > 1)
  			$page_description .= ' - Page ' . ((int)$_GET['Page']);
  	}
  	elseif (isset ($_GET['DepartmentId']) && ($_GET['DepartmentId'] != 'Amazon'))
  	{
  		$page_description = Catalog::GetDepartmentName($_GET['DepartmentId']);
  	...
  	elseif (isset ($_GET['ProductId']))
  	{
  		$page_description = Catalog::GetProductName($_GET['ProductId']);
  	}
  	elseif (isset ($_GET['SearchResults']))
  	{
  		// Display the search string
  		$page_description = trim(str_replace('-', ' ', $_GET['SearchString'])) . '" (';
  
  		// Display "all-words search " or "any-words search"
  		$all_words = isset ($_GET['AllWords']) ? $_GET['AllWords'] : 'off';
  
  		$page_description .= (($all_words == 'on') ? 'todas' : 'cualquiera de') . '- las palabras buscadas';
  ...
  {
    $page_title = 'Juassi: ' .
      'Temas y aplicaciones basadas en HTML5';
    ...
    elseif (isset ($_GET['SearchResults']))
    {
      ...
      $page_title .= (($all_words == 'on') ? 'todas' : 'cualquiera de') . '- las palabras buscadas';
